...
class UserscmsController extends AppController {
	...
	var $name = 'Userscms';
	var $uses =array('Userscms','City');
	var $components = array('Global','Email','SmtpEmail','Upload','Session');	
	function index() {
...
/* Ham save tai khoan tao */
 function add() {
			...
			if(!empty($_POST['email'])&& (!empty($_POST['security']))){
				...
				/*	$check = $this->Userscms->findByEmail($_POST['email']);
					$id=$check['Userscms']['id'];
					$content = "Chào mừng bạn đến với Website: ".DOMAIN."!,<br><br>
				Chúc mừng bạn đã trở thành thành viên của <b>batdongsanvtm.com</b> .<br><br>
				Vui lòng click vào đường link này <a href='".DOMAIN."Userscms/action_mail/".$action_mail."/".$id."'> <b>click vào đây </b></a> hoặc copy và chạy đường link này :".DOMAIN."Userscms/action_mail/".$action_mail."/".$id." để kích hoạt tài khoản.<br><br>
	...
	/*if ($this->SmtpEmail->smtpmailer($_POST['email'], ADDRESS_EMAIL_SEND_ACTIVE,FROM_NAME_EMAIL_ACTIVE, SUBJECT_EMAIL_ACTIVE,$content)){
					echo "<script>alert('".json_encode('Bạn hãy đăng nhập vào Email của bạn để hoàn thành quá trình đăng ký thành viên với chúng tôi')."');</script>";
				...
				/*	$check = $this->Userscms->find('all',array("conditions"=>array('Userscms.email'=>$_POST['email'],'Userscms.ck_admin'=>0)));
					$id=$check['0']['Userscms']['id'];
					$content = "Chào mừng bạn đến với Website: ".DOMAIN."!,<br><br>
				Chúc mừng bạn đã trở thành thành viên của <b>batdongsanvtm.com</b> .<br><br>
				Vui lòng click vào đường link này <a href='".DOMAIN."Userscms/action_mail/".$action_mail."/".$id."'> <b>click vào đây </b></a> hoặc copy và chạy đường link này :".DOMAIN."Userscms/action_mail/".$action_mail."/".$id." để kích hoạt tài khoản.<br><br>
				Nếu bạn có bất kỳ thắc mắc nào, hãy liên hệ với chúng tôi để nhận được sự hỗ trợ nhanh nhất.<br><br>
	...
	function district(){
		...
		$this->District->unbindModel(array('hasMany' => array('Immovablee','Userscms','Ward','Street')));
	...
	function action_mail($action_mail,$id){
		$this->Userscms->unbindModel(array('hasMany' => array('Immovable')));		
		$test=$this->Userscms->find('all',array("conditions"=>array("Userscms.active_key"=>$action_mail,"Userscms.id"=>$id)));
	...
	function processor_forgot_password(){
		$this->Userscms->unbindModel(array('hasMany' => array('Userscms')));
	...
	function activation($email,$new_pass) {
		$this->Userscms->unbindModel(array('hasMany' => array('Userscms')));
		$test=$this->Userscms->find('all',array("conditions"=>array("Userscms.email"=>$email)));
...
function unicode_convert($str){
		...
		return $this->City->find('all',array("conditions"=>array("City.id"=>$id),'limit'=>1));
